...
class SessionState:
    def __init__(self):
        ...
        self.current_topic_for_clarification = None  # Track which topic needs clarification

# Store states for different sessions (in production, use a proper session management system)
sessions = {}

def update_score_file(session_id, course, score):
    try:
        ...
        # Update the score for this session
...
def chat():
    ...
    if state.conversation_state == "awaiting_clarification":
        response = simplify_current_topic(state, message)
        state.conversation_state = "awaiting_next_topic_permission"  # After explaining, ask to move to next topic
    ...
    if state.conversation_state == "quiz_question":
        return jsonify({"response": handle_quiz_answer(state, message, session_id)})
...
def handle_quiz_answer(state, user_answer, session_id):
    index = state.current_quiz_index
    quiz_questions = state.quiz_questions
    ...
    if is_correct:
        state.score += 1  # Increment score for correct answer
    
    result = "✅ Correct!" if is_correct else f"❌ Incorrect. The correct answer is {correct}."
    state.current_quiz_index += 1

    # Update score file
    update_score_file(session_id, course, state.score)
...
def simplify_current_topic(state, clarification):
    ...
    try:
        simplified_response = client.chat.completions.create(
            model="llama3-8b-8192",
            messages=[
                {"role": "system", "content": f"You are an expert in {course}. Re-explain the specific part of '{topic}' that the student didn't understand: '{clarification}'. Keep it simple and clear."},
                {"role": "user", "content": f"Please explain this part in simpler terms: {clarification}"}
            ],
            temperature=0.7
        )
        simple_explanation = simplified_response.choices[0].message.content.strip()
